...
import static BobcatLib.Subsystems.Vision.limelight.networktables.LimelightUtils.orientation3dToArray;
import static BobcatLib.Subsystems.Vision.limelight.networktables.LimelightUtils.pose3dToArray;
import static BobcatLib.Subsystems.Vision.limelight.networktables.LimelightUtils.translation3dToArray;
...
public class LimelightSettings {
  ...
  /**
   * Stream mode, 0 = Side-by-side, 1 = Picture-in-Picture (second in corner), 2 =
   * Picture-in-Picture (primary in corner)
   */
   ...
   * cost of potentially reduced detection range. Valid values ar [0 (pipeline control), 1 (no
   * downscale), 2, 3, 4]
   */
  ...
  /**
   * Create a {@link LimelightSettings} object with all configurable features of a {@link
   * Limelight}.
   *
  ...
  public LimelightSettings(Limelight camera) {
    ...
    fiducial3DOffset =
        limelightTable.getDoubleArrayTopic("fiducial_offset_set").getEntry(new double[0]);
    cameraToRobot =
        limelightTable.getDoubleArrayTopic("camerapose_robotspace_set").getEntry(new double[0]);
    fiducialIDFiltersOverride =
        limelightTable.getDoubleArrayTopic("fiducial_id_filters_set").getEntry(new double[0]);
  }
   ...
   * <p>This method changes the Limelight - normally immediately.
   *
   * @param offset {@link Pose3d} of the {@link Limelight} with the {@link
