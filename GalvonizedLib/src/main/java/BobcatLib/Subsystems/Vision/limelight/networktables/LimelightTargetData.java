...
import static BobcatLib.Subsystems.Vision.limelight.networktables.LimelightUtils.toPose3D;
...
public class LimelightTargetData {
  ...
  /**
   * Array containing [targetValid, targetCount, targetLatency, captureLatency, tx, ty, txnc, tync,
   * ta, tid, targetClassIndexDetector, targetClassIndexClassifier, targetLongSidePixels,
   * targetShortSidePixels, targetHorizontalExtentPixels, targetVerticalExtentPixels,
   * targetSkewDegrees]
   */
  ...
  /**
   * {@link Pose3d} object representing the target's position and orientation relative to the
   * camera.
   */
  private DoubleArrayEntry target2CameraPose;
  /**
   * {@link Pose3d} object representing the camera's position and orientation relative to the
   * target.
   */
  ...
  public LimelightTargetData(Limelight camera) {
    ...
    target2RobotPose =
        limelightTable.getDoubleArrayTopic("targetpose_robotspace").getEntry(new double[0]);
    target2CameraPose =
        limelightTable.getDoubleArrayTopic("targetpose_cameraspace").getEntry(new double[0]);
    camera2TargetPose =
        limelightTable.getDoubleArrayTopic("camerapose_targetspace").getEntry(new double[0]);
    robot2TargetPose =
        limelightTable.getDoubleArrayTopic("botpose_targetspace").getEntry(new double[0]);
  }
   ...
   *
   * @return {@link Pose3d} object representing the robot's position and orientation relative to the
   *     target
   */
   ...
   *
   * @return {@link Pose3d} object representing the target's position and orientation relative to
   *     the camera
   */
